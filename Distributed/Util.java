...
public class Util {
    ...
    public static byte[] marshall(int x) {
        ...
        byte[] arrayByte = new byte[] { (byte) ((x >> 24) & 0xff), (byte) ((x >> 16) & 0xff), (byte) ((x >> 8) & 0xff),
                (byte) ((x >> 0) & 0xff) };
        return arrayByte;
    }
    ...
    public static int unmarshallInt(byte[] x) {
        ...
        if (x == null || x.length != 4)
            ...
            int intValue = (int) ((0xff & x[0]) << 24 | (0xff & x[1]) << 16 | (0xff & x[2]) << 8 | (0xff & x[3]) << 0);
            return intValue;
        }
    }
    ...
    public static byte[] marshall(long x) {
        // Bit-shifting into an array
        byte[] arrayByte = new byte[] { (byte) ((x >> 56) & 0xffL), (byte) ((x >> 48) & 0xffL),
                (byte) ((x >> 40) & 0xffL), (byte) ((x >> 32) & 0xffL), (byte) ((x >> 24) & 0xffL),
                (byte) ((x >> 16) & 0xffL), (byte) ((x >> 8) & 0xffL), (byte) ((x >> 0) & 0xffL) };
        return arrayByte;
    }

    // Demarshalling Long
    public static long unmarshallLong(byte[] x) {
        // Ensure that the byte array given is not empty or more/less than 8B (LONG
        // size)
        if (x == null || x.length != 8)
            return 0x0;
        else {
            long longValue = (long) ((0xffL & x[0]) << 56 | (0xffL & x[1]) << 48 | (0xffL & x[2]) << 40
                    | (0xffL & x[3]) << 32 | (0xffL & x[4]) << 24 | (0xffL & x[5]) << 16 | (0xffL & x[6]) << 8
                    | (0xffL & x[7]) << 0);
            return longValue;
        }
    }
    ...
    public static String unmarshallString(byte[] s) {
        ...
        for (int c_index = 0; c_index < arrayChars.length; c_index++) {
            arrayChars[c_index] = (char) ((0xff & s[2 * c_index]) << 8 | (0xff & s[2 * c_index + 1]) << 0);
        }
    ...
    // TODO
    public static String getDate(byte[] payload, int payloadSize) {
        int stringSize = payloadSize - 8;
        byte[] date = new byte[stringSize];
        System.arraycopy(payload, 8, date, 0, date.length);
        return unmarshallString(date);
    }
